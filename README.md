...
9. [SEO e metadata](#9-seo-e-metadata)
...
Portal de base de conhecimento público da **RBCont** (`rbcont.com.br`), empresa brasileira de hospedagem WordPress de alta performance e VPS.

**URL de produção:** `https://ajuda.rbcont.com.br`  
**Referência de mercado:** `https://ajuda.staycloud.com.br` (estrutura de categorias semelhante, stack diferente)

**O que o sistema faz:**
- Exibe artigos de documentação e tutoriais organizados por categoria para os clientes da RBCont
- Todo conteúdo é público e indexável pelo Google
- Uma área administrativa protegida permite que editores criem, editem e publiquem artigos sem conhecimento técnico
...
- Não tem integração com o painel de cliente da RBCont (é um produto independente)
...
# NextAuth
NEXTAUTH_URL="https://ajuda.rbcont.com.br"
NEXTAUTH_SECRET="[gerar: openssl rand -base64 32]"

# App
NEXT_PUBLIC_SITE_URL="https://ajuda.rbcont.com.br"
```
...
**Article** — `id, type (TEXT|VIDEO), title, slug, content (HTML), excerpt, videoUrl, status, views, helpful, notHelpful, featured, categoryId, authorId, publishedAt`
...
```nginx
location /uploads/ {
    alias /var/www/rbcont-ajuda/public/uploads/;
    expires 30d;
...
> **Otimização SEO foi removida do admin.** Não há mais campos `metaTitle`/`metaDesc` na model `Article` nem na UI do editor. O `<title>` e o `<meta description>` são gerados automaticamente a partir de `article.title` e `article.excerpt` por `lib/seo.ts`.

### generateMetadata em rotas públicas
...
`buildArticleMetadata` produz `{ title, description, alternates.canonical }`. Para customizar `<title>` ou `<description>` de um artigo específico, edite os campos `title` e `excerpt` direto — não há override separado.

### Acesso atualmente é login-only

O middleware bloqueia visitantes anônimos em todas as rotas. Por isso:
- `app/layout.tsx` define `robots: { index: false, follow: false }` no nível root
- `app/sitemap.ts` retorna array vazio

Quando a leitura pública for liberada para visitantes anônimos, remover esses bloqueios e popular o sitemap.
...
```bash
npm run build
pm2 start npm --name "rbcont-ajuda" -- start
pm2 save
```
...
server {
    listen 80;
    server_name ajuda.rbcont.com.br;
...
server {
    ...
    ssl_certificate     /etc/letsencrypt/live/ajuda.rbcont.com.br/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/ajuda.rbcont.com.br/privkey.pem;
...
*Documento mantido pelo time RBCont. Atualizar este README sempre que houver mudança de stack, modelo de dados ou decisão de arquitetura.*
